...
def get_scene_pcd():
    ...
    with open('/mnt/storage/hand_to_eye_calib_mat.txt', 'r') as infile:
...
TARGET = 'pallet'#'pallet' #'container2_robot2' # 'container' 'bagger' 'tool' 'conveyor'
TARGET_PCD = 'pallet'#'pallet'#'container2_robot2' # 'container' 'Bagging Machine' 'EPick' 'conveyor'
...
Pallet = True
if Pallet:
    T_guess = np.array([[ 1.        ,  0.        ,  0.        , -1.0      ],
    [ 0.        , -0.64278761, -0.76604444,  0.8       ],
    [-0.        ,  0.76604444, -0.64278761,  1.8       ],
    [ 0.        ,  0.        ,  0.        ,  1.        ]])     
    T_guess = np.save('T_guess.npy',T_guess )  
else:
    # initial pose for pose estimation
    theta_deg = 135
    ...
    [-0.        ,  np.sin(theta_rad), np.cos(theta_rad),  0.65       ], [ 0.        ,  0.        ,  0.        ,  1.        ]])  
...
while 1:
            ...
            for (sigma_i, sigma) in enumerate(sigmas):
                ...
                for noise_i in range(N_NOISE_REPEAT):
                    ...
                    if VIZ:
                        ...
                        #meshcat.SetObject(f"{TARGET}_scene", scene_drake, point_size=0.005, rgba=Rgba(0, 0, 0, 1))
                    ...
                    #     meshcat.SetObject(f"{TARGET}_model_detected", model_drake, point_size=0.02, rgba=Rgba(1, 1, 0, 0.5))
                      #  meshcat.SetTransform(f"{TARGET}_model_detected", RigidTransform(T_guess))

                    is_same = check_transform_error(T_guess,T_teaser)
                    # print(scene_val"is_same: ",is_same)
                    if is_same ==  True:
                        T_guess = T_teaser
                    # ipdb.set_trace()
                    # fine tuning via icp
    ...
    print("x:",x,"y:",y,"rz:",rz)
    np.savetxt('/mnt/storage/target_to_hand_mat.txt', T_b_t)
    print("target pose in robot base transformation:")
    print(T_b_t)
    # ipdb.set_trace()
