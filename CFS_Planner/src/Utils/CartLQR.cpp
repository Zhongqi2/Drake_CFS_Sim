...
void CartesianLQR2ndOrder::Setup()
{
    ...
    cfslib::math::SetQuadMatFromEigen(G, G_eigen);
    ...
    cfslib::math::SetQuadMatFromEigen(CE, -Aeq.transpose());
    ...
    Aineq = cfslib::math::EigenVcat(Aineq, Aineq3);
    ...
    Aineq = cfslib::math::EigenVcat(Aineq, Aineq4);

    Eigen::MatrixXd Aineq5(N*cartd, N*ud+N*sd);
    Aineq5 << -Ivbar*Bbar, Eigen::MatrixXd::Zero(N*cartd, N*sd);
    Aineq = cfslib::math::EigenVcat(Aineq, Aineq5);

    cfslib::math::SetQuadMatFromEigen(CI, -Aineq.transpose());
...
std::pair<Eigen::MatrixXd, Eigen::MatrixXd> CartesianLQR2ndOrder::Solve(const Eigen::VectorXd& xk)
{
    ...
    */
    ...
    cfslib::math::SetQuadVecFromEigen(g0, g0_eigen.transpose());
    #ifdef DEBUG_PRINT
        std::cout << " ------------------- g0_eigen ----------------- \n" << g0_eigen.transpose() << "\n";
    #endif
    ...
    cfslib::math::SetQuadVecFromEigen(ce0, beq);
    #ifdef DEBUG_PRINT
        std::cout << " ------------------- beq ----------------- \n" << beq << "\n";
    #endif
    ...
    bineq1 << -IRbar*fbar;
    bineq = cfslib::math::EigenVcat(bineq, bineq1);

    Eigen::MatrixXd bineq2(N*sd, 1);
    bineq2 << IRbar*fbar;
    bineq = cfslib::math::EigenVcat(bineq, bineq2);

    Eigen::MatrixXd bineq3(2*N*ud+N*sd, 1);
    bineq3 << bubar,
              bubar,
              Eigen::MatrixXd::Zero(N*sd, 1);
    bineq = cfslib::math::EigenVcat(bineq, bineq3);

    Eigen::MatrixXd bineq4(N*cartd, 1);
    bineq4 = bvbar - Ivbar*fbar;
    bineq = cfslib::math::EigenVcat(bineq, bineq4);

    Eigen::MatrixXd bineq5(N*cartd, 1);
    bineq5 = bvbar + Ivbar*fbar;
    bineq = cfslib::math::EigenVcat(bineq, bineq5);

    cfslib::math::SetQuadVecFromEigen(ci0, bineq);
    #ifdef DEBUG_PRINT
        std::cout << " ------------------- bineq ----------------- \n" << bineq << "\n";
    #endif
    ...
    cfslib::math::SetQuadVecFromEigen(var, Eigen::MatrixXd::Zero(N*ud+N*sd, 1));
    ...
    cfslib::math::SetEigenMatFromQuad(result, var);
    ...
    const Eigen::MatrixXd& Q, const Eigen::MatrixXd& S, const Eigen::MatrixXd& R)
{
    ...
    */
    ...
    cfslib::math::SetQuadMatFromEigen(G, Bbar.transpose()*Qbar*Bbar+Rbar);
    cfslib::math::SetQuadVecFromEigen(g0, (fbar.transpose()*Qbar*Bbar).transpose());

    CE.resize(N*ud, 0);
    ce0.resize(0);

    cfslib::math::SetQuadMatFromEigen(CI, cfslib::math::EigenVcat(-Lubar, Lubar).transpose());
    cfslib::math::SetQuadVecFromEigen(ci0, cfslib::math::EigenVcat(bubar, bubar));

    cfslib::math::SetQuadVecFromEigen(Uk_quad, Eigen::MatrixXd::Zero(N*ud, 1));

    double cost = quadprogpp::solve_quadprog(G, g0, CE, ce0, CI, ci0, Uk_quad);

    Eigen::VectorXd Uk;
    cfslib::math::SetEigenMatFromQuad(Uk, Uk_quad);
